...
def parse_instructions(instructions):
    for line in instructions.splitlines():
        match = re.match(r"([a-z ]+) (\d+),(\d+) through (\d+),(\d+)", line)
        if not match:
            print("Don't understand this line: {!r}".format(line))
            continue
        coords = map(int, match.groups()[1:])
        yield (match.group(1),) + tuple(coords)

def range2d_inc(x0, y0, x1, y1):
    if x0 > x1:
        x0, x1 = x1, x0
    if y0 > y1:
        y0, y1 = y1, y0
...
def coords_on(instructions):
    on = set()
    for op, *coords in parse_instructions(instructions):
        do_op = op
        for coord in range2d_inc(*coords):
            if op == "toggle":
                do_op = "turn off" if (coord in on) else "turn on"
            if do_op == "turn on":
                on.add(coord)
            elif do_op == "turn off":
                on.discard(coord)
...
print("Answer: {} lights are on".format(len(coords_on(INPUT))))

def elvish_lights(instructions):
...
print("Answer: {} total brightness".format(sum(elvish_lights(INPUT).values())))
