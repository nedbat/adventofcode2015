...
def decode(literal):
    ...
    chars = re.findall(r'''\\\\|\\\"|\\x[0-9a-f][0-9a-f]|.''', literal[1:-1])
    return chars
...
def delta(input):
    ...
    for line in input.splitlines():
        code_chars += len(line)
        memory_chars += len(decode(line))
    return code_chars - memory_chars

print("Test: delta = {}".format(delta(TEST)))
print("Answer: delta = {}".format(delta(INPUT)))
...
def delta2(input):
    ...
    for line in input.splitlines():
        code_chars += len(line)
        encoded_chars += len(encode(line))
    return encoded_chars - code_chars

print("Test: delta2 = {}".format(delta2(TEST)))
print("Answer: delta2 = {}".format(delta2(INPUT)))

# The cheater way:
print("Answer 1: {}".format(sum(len(l) - len(eval(l)) for l in INPUT.splitlines())))
# oops, doesn't work because Python uses single quotes for repr...
print("Answer 2: {}".format(sum(len(repr(l)) - len(l) for l in INPUT.splitlines())))
